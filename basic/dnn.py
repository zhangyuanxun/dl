...
class DNN(object):
    ...
    def __init__(self, num_inputs, num_outputs):
        ...
        self.weights = {
            ...
            'out': tf.Variable(tf.random.normal(shape=[self.num_hidden2, self.num_outputs], stddev=0.1)),
        }

        self.biases = {
            'b1': tf.Variable(tf.random.normal(shape=[self.num_hidden1])),
            'b2': tf.Variable(tf.random.normal(shape=[self.num_hidden2])),
            'out': tf.Variable(tf.random.normal(shape=[self.num_outputs])),
        }
...
if __name__ == '__main__':
    ...
    model = DNN(num_inputs=num_inputs, num_outputs=num_outputs)
